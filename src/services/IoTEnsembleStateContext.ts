...
import { StateContext } from "../src.deps.ts";
import {
  ColdQueryDataTypes,
...
} from "./iot-ensemble-state.models.ts";
...
export class IoTEnsembleStateContext extends StateContext<IoTEnsembleState> {
  ...
  public ListAllDeviceNames(
    childEntLookup: string,
    filter: string,
  ): Promise<object> {
  ...
  protected gtagEvent(stateAction: string, eventArgs: any) { //GtagCustomParams) {
    ...
    //   ...eventArgs,
    // });
    console.log([stateAction, eventArgs]);
  }
  ...
  protected diffState(reqState: any) {
    ...
    stateKeys.forEach((stateKey) => {
      ...
      const oldVal = JSON.stringify((this.oldState as any)[stateKey]);

      if (reqVal !== oldVal) {
        (diffed as any)[stateKey] = reqState[stateKey];
      }
    });
